...
// ------------------------------------------------------------------------	
	...
	while( ( fscanf( inl, "%d%d%d%d\n", &m, &l, &n, &nrep ) != EOF ) ){
		...
		nDIM = calcdim(n);
#if 1
		posix_memalign(&a, 4096, mDIM*lDIM*sizeof( REAL *) );
		posix_memalign(&b, 4096, lDIM*nDIM*sizeof( REAL *) );
		posix_memalign(&c, 4096, mDIM*nDIM*sizeof( REAL *) );
      
		for(i=0;i<mDIM*lDIM;i++)
			posix_memalign(&a[i], 4096, BSIZE*BSIZE*sizeof(REAL));

		for(i=0;i<lDIM*nDIM;i++)
			posix_memalign(&b[i], 4096, BSIZE*BSIZE*sizeof(REAL));

		for(i=0;i<mDIM*nDIM;i++)
			posix_memalign(&c[i], 4096, BSIZE*BSIZE*sizeof(REAL));
